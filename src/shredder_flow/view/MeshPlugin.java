...
public class MeshPlugin extends Plugin {
	...
	private double getMaxFieldVectorLength(TriangleList triangles) {
		double maxFieldVector = 0;
		for (Triangle triangle : triangles) {
			if (maxFieldVector < triangle.getFieldVector().length()) {
				maxFieldVector = triangle.getFieldVector().length();
			}
		}
		return maxFieldVector;
	}

	private double getMinSideLength(TriangleList triangles) {
		double minLength = 0;
		for (Triangle triangle : triangles) {
			if (minLength > getTriangleLength(triangle) || minLength == 0) {
				minLength = getTriangleLength(triangle);
			}
		}
		return minLength;
	}
	...
	private SceneComponent getFieldVectorSceneComponent(Triangle triangle,
		...
		double c = Math.cos(angle) * minSideLength / 8;
	...
	private Vertex getRootVertex(Vertex center, double[] normedFieldVec) {
		Vertex vertex = new Vertex(center.getX() - normedFieldVec[0],
				center.getY() - normedFieldVec[1]);
		return vertex;
	}
	...
	private double[] getNormedFieldVector(FieldVector fieldVec, double l) {
		...
		normedVector[0] = (fieldVec.x) * l;
		normedVector[1] = (fieldVec.y) * l;
		return normedVector;
	...
	private Vertex getTriangleCenter(Triangle triangle) {
		...
		Vertex center = new Vertex(
				(vertex0.getX() + vertex1.getX() + vertex2.getX()) / 3,
				(vertex0.getY() + vertex1.getY() + vertex2.getY()) / 3);
		return center;
	}
	...
	public void install(Controller c) throws Exception {
		super.install(c);
		SceneComponent parentSceneComponent = c.getPlugin(Java2DView.class)
				.getViewer2D().getRoot();
